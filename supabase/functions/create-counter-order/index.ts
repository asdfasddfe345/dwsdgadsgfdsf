...
interface CreateBody {
  ...
  detectedGpsLat?: number;
  detectedGpsLng?: number;
...
Deno.serve(async (req: Request) => {
  ...
  try {
    ...
    const houseNumber = body.houseNumber?.trim() || null;
    const buildingName = body.buildingName?.trim() || null;
    const floorNumber = body.floorNumber?.trim() || null;
    const landmark = body.landmark?.trim() || null;
    const deliveryInstructions = body.deliveryInstructions?.trim() || null;
    const detectedGpsLat = typeof body.detectedGpsLat === "number" && Number.isFinite(body.detectedGpsLat) ? body.detectedGpsLat : null;
    const detectedGpsLng = typeof body.detectedGpsLng === "number" && Number.isFinite(body.detectedGpsLng) ? body.detectedGpsLng : null;
    const addressConfidence = typeof body.addressConfidence === "number" ? body.addressConfidence : null;
    const savedAddressId = body.savedAddressId?.trim() || null;
    ...
    const orderInsert: AppOrderInsert = {
      ...
      house_number: fulfillment.orderType === "delivery" ? houseNumber : null,
      building_name: fulfillment.orderType === "delivery" ? buildingName : null,
      floor_number: fulfillment.orderType === "delivery" ? floorNumber : null,
      landmark: fulfillment.orderType === "delivery" ? landmark : null,
      delivery_instructions: fulfillment.orderType === "delivery" ? deliveryInstructions : null,
      detected_gps_lat: fulfillment.orderType === "delivery" ? detectedGpsLat : null,
      detected_gps_lng: fulfillment.orderType === "delivery" ? detectedGpsLng : null,
      address_confidence: fulfillment.orderType === "delivery" ? addressConfidence : null,
      saved_address_id: fulfillment.orderType === "delivery" ? savedAddressId : null,
