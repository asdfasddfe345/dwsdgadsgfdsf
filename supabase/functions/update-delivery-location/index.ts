...
Deno.serve(async (req: Request) => {
  ...
  try {
    ...
    if (!orderId || lat == null || lng == null) {
      return json({ error: "orderId, lat, and lng are required" }, 400);
    }
    ...
    // Verify the order is a delivery order that's out for delivery
    ...
    const { error: updateError } = await adminClient
      ...
      .update({
        ...
        delivery_partner_location_updated_at: new Date().toISOString(),
      })
      .eq("id", orderId);
